...
set :repository, 'git@github.com:dalezhang/books.git'
...
set :shared_dirs, fetch(:shared_dirs, []).push('log', 'public/packs', 'config/rsa')
...
task :setup do
  ...
  command %{touch "#{fetch(:shared_path)}/tmp/sockets/books.sock"}
end
